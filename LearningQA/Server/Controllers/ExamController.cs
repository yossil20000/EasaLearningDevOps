...
namespace LearningQA.Server.Controllers
{
    public class ExamController : ApiControllerBase
    {
        ...
        [HttpGet(Name = "/CombineExamByIds")]
        [SwaggerResponse((int)System.Net.HttpStatusCode.OK, "ExamModel", typeof(ExamModel))]
        [SwaggerResponse((int)System.Net.HttpStatusCode.NotFound, "ExamModel", typeof(ExamModel))]
        public async Task<ActionResult<ExamModel>> CombineExamByIds([FromQuery] ListOfIds<int> listOfIds, QuestionListFilter questionListFilter, CancellationToken cancellationToken = default)
        {
            var result = await _mediator.Send(new GetExamCombineByIdCommand(listOfIds.Ids,questionListFilter), cancellationToken);   
            return this.FromResult(result);
        }
