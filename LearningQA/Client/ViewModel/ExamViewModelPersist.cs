...
namespace LearningQA.Client.ViewModel
{
	...
	public class ExamViewModelPersist : PersistanceBase , IDisposable
	{
		...
		public QuestionListFilter GetExamQuestionFilter { get; set; } = QuestionListFilter.All;
		private void OnQuestionListFilter()
		{
			...
			FilteredAnsware = CurrentTest.Answers.Where(x => 
			(
			questionListFilter.IsFlagSet(QuestionListFilter.All) ||
			(
			(isWrong ? !x.IsCorrect : true) &&
			(isMarked ? x.IsMarked : true) &&
			(isNotAnswered ? !x.IsAnswered : true )
			)
			)).ToList();
